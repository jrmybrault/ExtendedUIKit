...
extension UIView {
    ...
    public func constraintSize(to size: CGSize) {
        constraintWidth(to: size.width)
        constraintHeight(to: size.height)
    }

    @discardableResult
    public func constraintWidth(to view: UIView, ratio: CGFloat = 1) -> NSLayoutConstraint {
        return widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: ratio).activate()
    }

    @discardableResult
    public func constraintHeight(to view: UIView, ratio: CGFloat = 1) -> NSLayoutConstraint {
        return heightAnchor.constraint(equalTo: view.heightAnchor, multiplier: ratio).activate()
    }
}
